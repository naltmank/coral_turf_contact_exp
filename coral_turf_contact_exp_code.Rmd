...
comb <- read.csv(here::here("data", "coral_algal_contact_data_combined.csv"), stringsAsFactors = T) # read data

# subset data from coral-algal contact experiment
c_exp <- subset(comb, Experiment == "Coral_algal_contact")
...
```{r week 1 stats}
week1 <- subset(c_exp, Week == 1) # subset initial week 1 data
...
ggplot(data = week2) +
 ...
 geom_text(data = week2_graph, aes(x = Algal.treatment, y = 0.72,
           ...
           size = 12) +
  geom_text(aes(x = 0.7, y = 0.15, label = "Algae: P < 0.01\nCoral: P = 0.01\nCoral X Algae: P < 0.01"), hjust = 0, size = 12)+
...
surface <- subset(comb, Experiment == "Summer_allelopathy")
...
winter <- subset(comb, Experiment == "Winter_allelopathy")
...
vegan::permutest(porites_dispr) # F = 0.8, P = 0.55
...
## Figure S5 - DeSeq Heatmap

```{r acropora deseq summary matrix, fig.height = 10, fig.width= 10}
treatment_order_heatmap <- c('Empty', 'Macroalgae mimic', 'Turf mimic', 'Amansia',
                              'Sargassum', 'S nigricans', 'S punctatus')
...
(acropora_heatmap <- ggplot(data = melted_acropora_deseq, aes(Var2, Var1, fill = value))+
 ...
 geom_text(aes(Var2, Var1, label = value), color = "black", size = 8) +
      scale_x_discrete(limits = treatment_order_heatmap, 
                       labels = c("Zip-tie",
                              "Turf mimic",
                              "Macroalgal mimic",
                              expression(italic("Amansia")),
                              ...
                              expression(italic("S. nigricans")),
                              expression(italic("S. punctatus")))) +
          scale_y_discrete(limits = treatment_order_heatmap, 
                       labels = c("Zip-tie",
                              "Turf mimic",
                              "Macroalgal mimic",
                              expression(italic("Amansia")),
...
```{r heatmap panel, fig.height = 22, fig.width= 14}

grid.arrange(arrangeGrob(acropora_heatmap , porites_heatmap, nrow = 2)
             )

ggarrange(acropora_heatmap, porites_heatmap, ncol=1, nrow=2, heights = c(1,1)) # fix legend position in photoshop
```
...
(acropora_bray_plot_2 <-
  ...
  geom_text_repel(data = acropora_genus_df, aes(x = PCoA1, y = PCoA2, label = Genus), 
...
(porites_bray_plot_2 <-
  ...
  geom_segment(data = porites_genus_df, aes(x = 0, y = 0, xend = PCoA1, yend = PCoA2),
                 arrow = arrow(length = unit(0.2, "cm")), color = "black") +
  geom_text_repel(data = porites_genus_df, aes(x = PCoA1, y = PCoA2, label = Genus), 
            size = 6, color = "black", vjust = -0.5))

```
### Plot Figure 5 R2
```{r microbial diversity panel graph R2, fig.height= 30, fig.width=20}
ggarrange(acropora_bray_plot_2, 
          porites_bray_plot_2, 
          acropora_observed_ggplot + theme(axis.text.x=element_blank()),
          porites_observed_ggplot + theme(axis.text.x=element_blank()),
          acropora_adiv_ggplot,
...
# New Figure 25 - Binary Jaccard with species vectors

```{r jaccard with vectors}
# Extract scores
acropora_jaccard_scores <- acropora_jaccard$vectors[, 1:2]

# extract genera data
acropora_jaccard_genus_table <- tax_glom(acropora_binary, taxrank = "Genus")

acropora_jaccard_genera_data <- as.data.frame(t(otu_table(acropora_jaccard_genus_table))) 

acropora_jaccard_taxa_names <- as.data.frame(tax_table(acropora_jaccard_genus_table))$Genus  # Extract Genus names
colnames(acropora_jaccard_genera_data) <- acropora_jaccard_taxa_names  # Replace otus as columns with Genus names

acropora_jaccard_genera_data_subset <- acropora_jaccard_genera_data[,colnames(acropora_jaccard_genera_data) 
                                                                    %in% acropora_important_genera ]

# Fit mean centroids for genera of interest
acropora_jaccard_fit <- envfit(acropora_jaccard_scores, acropora_genera_data_subset, perm = 999)

# extract vectors
acropora_jaccard_genus_vectors <- scores(acropora_jaccard_fit, display = "vectors")

# convert to dataframe
acropora_jaccard_genus_df <- data.frame(Genus = rownames(acropora_jaccard_genus_vectors),
                       PCoA1 = acropora_genus_vectors[, 1], 
                       PCoA2 = acropora_genus_vectors[, 2])

(acropora_jaccard_plot_2 <-
  acropora_jaccard_plot + 
  geom_segment(data = acropora_jaccard_genus_df, aes(x = 0, y = 0, xend = PCoA1, yend = PCoA2),
                 arrow = arrow(length = unit(0.2, "cm")), color = "black") +
  geom_text_repel(data = acropora_jaccard_genus_df, aes(x = PCoA1, y = PCoA2, label = Genus), 
            size = 6, color = "black", vjust = -0.5)) 

# Repeat with porites ----
porites_jaccard_scores <- porites_jaccard$vectors[, 1:2]

# extract genera data
porites_jaccard_genus_table <- tax_glom(porites_binary, taxrank = "Genus")

porites_jaccard_genera_data <- as.data.frame(t(otu_table(porites_jaccard_genus_table))) 

porites_jaccard_taxa_names <- as.data.frame(tax_table(porites_jaccard_genus_table))$Genus  # Extract Genus names
colnames(porites_jaccard_genera_data) <- porites_jaccard_taxa_names  # Replace otus as columns with Genus names

porites_jaccard_genera_data_subset <- porites_jaccard_genera_data[,colnames(porites_jaccard_genera_data) 
                                                                    %in% porites_important_genera ]

# Fit mean centroids for genera of interest
porites_jaccard_fit <- envfit(porites_jaccard_scores, porites_genera_data_subset, perm = 999)

# extract vectors
porites_jaccard_genus_vectors <- scores(porites_jaccard_fit, display = "vectors")

# convert to dataframe
porites_jaccard_genus_df <- data.frame(Genus = rownames(porites_jaccard_genus_vectors),
                       PCoA1 = porites_genus_vectors[, 1], 
                       PCoA2 = porites_genus_vectors[, 2])

(porites_jaccard_plot_2 <-
  porites_jaccard_plot + 
  geom_segment(data = porites_jaccard_genus_df, aes(x = 0, y = 0, xend = PCoA1, yend = PCoA2),
                 arrow = arrow(length = unit(0.2, "cm")), color = "black") +
  geom_text_repel(data = porites_jaccard_genus_df, aes(x = PCoA1, y = PCoA2, label = Genus), 
            size = 6, color = "black", vjust = -0.5)) 

```
```{r jaccard diversity panel graph R2, fig.height=10, fig.width=20}
ggarrange(acropora_jaccard_plot_2, 
          porites_jaccard_plot_2, ncol=2, nrow=1, widths = c(1,1), common.legend = TRUE,
          legend="bottom", legend.grob = get_legend(acropora_bray_plot)) 
```
# New Figure S4 - class level viz
```{r stacked bar chart, fig.height = 10, fig.width=25}

# extract class data
acropora_class_table <- tax_glom(acropora_rel_abund, taxrank = "Class")

acropora_class_data <- as.data.frame(t(otu_table(acropora_class_table))) 

acropora_class_names <- as.data.frame(tax_table(acropora_class_table))$Class  # Extract Genus names
colnames(acropora_class_data) <- acropora_class_names  # Replace otus as columns with Genus names
acropora_class_data$Algal.trt <- acropora_adiv$Algal.trt

# extract class data
porites_class_table <- tax_glom(porites_rel_abund, taxrank = "Class")

porites_class_data <- as.data.frame(t(otu_table(porites_class_table))) 

porites_class_names <- as.data.frame(tax_table(porites_class_table))$Class  # Extract Genus names
colnames(porites_class_data) <- porites_class_names  # Replace otus as columns with Genus names

porites_class_data$Algal.trt <- porites_adiv$Algal.trt

# find classes common across both dataframes
common_classes <- intersect(colnames(acropora_class_data)[colnames(acropora_class_data) != "Algal.trt"],
                            colnames(porites_class_data)[colnames(porites_class_data) != "Algal.trt"])

# subset appropriately
acropora_common <- acropora_class_data %>%
  dplyr::select(Algal.trt, all_of(common_classes))

porites_common <- porites_class_data %>%
  dplyr::select(Algal.trt, all_of(common_classes))

combined_data <- base::rbind(acropora_common, porites_common)

# pivot longer for graphing
long_class_data <- combined_data %>%
  rownames_to_column(var = "SampleID") %>%  # Optional: keep track of sample IDs
  pivot_longer(
    cols = -c(SampleID, Algal.trt),  # All columns except "SampleID" and "Treatment" will be pivoted
    names_to = "Class",              # Create a "Class" column for microbial classes
    values_to = "Abundance"          # Relative abundances go into the "Abundance" column
  )

# extract the top 20 most abundant classes
top_classes <- long_class_data %>%
  group_by(Class) %>%
  summarise(Total_Abundance = sum(Abundance, na.rm = TRUE)) %>%
  arrange(desc(Total_Abundance)) %>%
  slice_head(n = 20) %>%  # Select the top 20 classes
  pull(Class)  # Extract the class names

# Filter long_class_data to only include top class data
top_class_data <- long_class_data %>%
  filter(Class %in% top_classes)

# get the relative abundance for all "other" classes
other_rel_abun <- 1 - rowSums(combined_data[ , colnames(combined_data) %in% top_classes])

# Coerce into a df the same format as the long_class_data
other_rel_abun_df <-data.frame(
  SampleID = names(other_rel_abun),
  Algal.trt = long_class_data$Algal.trt[match(names(other_rel_abun), long_class_data$SampleID)],  # Match treatment to sample
  Class = "Other",  # Class is set to "Other"
  Abundance = other_rel_abun  # The abundance values from the vector
)

# Recombine to create new long_class_data
long_class_data <- bind_rows(top_class_data, other_rel_abun_df)


# Add species column
long_class_data <- long_class_data %>%
  mutate(Coral = ifelse(grepl("^A", SampleID), "Acropora", "Porites"))  



# make other always come out first
long_class_data <- long_class_data %>%
  mutate(Class = ifelse(Class %in% top_classes, Class, "Other")) %>%
  mutate(Class = factor(Class, levels = c("Other", top_classes)))  # Ensure "Other" is the first level

# For plot colours
colors <- c(rainbow(20), "grey")  # Assign rainbow colors to top 19 classes, grey to "Other"
names(colors) <- c(top_classes, "Other")  # Correctly map colors to the classes

acropora_class_data_2 <- subset(long_class_data, Coral == "Acropora")
porites_class_data_2 <- subset(long_class_data, Coral == "Porites")

acropora_class_data_2$Algal.trt <- as.factor(acropora_class_data_2$Algal.trt)
facet_treatment_order <- c("Empty", "Macroalgae mimic", "Turf mimic", "Amansia", "Sargassum", "S nigricans","S punctatus")

# Will have to add italics in photoshop - not working here for some reasong
(acropora_class_plot <-
  ggplot(acropora_class_data_2, aes(x = SampleID, y = Abundance, fill = Class)) +
  geom_bar(stat = "identity", position = "stack") +  # Stacked bar plot
  facet_wrap(~ factor(Algal.trt, levels = facet_treatment_order),
  #           labeller = as_labeller(facet_labels),
             scales = "free_x", nrow = 1, strip.position = "bottom") +  # Facet by species 
  guides(fill = guide_legend(ncol = 1)) +
  scale_y_continuous(limits = c(0,1.01), expand = c(0, 0)) + # remove blank space
...
(porites_class_plot <-
  ggplot(porites_class_data_2, aes(x = SampleID, y = Abundance, fill = Class)) +
  geom_bar(stat = "identity", position = "stack") +  # Stacked bar plot
  facet_wrap(~ factor(Algal.trt, levels = facet_treatment_order),
#             labeller = as_labeller(facet_labels),
             scales = "free_x", nrow = 1, strip.position = "bottom") +  # Facet by species 
...
```{r stacked class graph, fig.height = 25, fig.width=25}
ggarrange(acropora_class_plot, NULL, # Null to add space
          porites_class_plot, ncol=1, nrow=3, widths = c(1,1,1), heights = c(1, 0.1, 1), common.legend = TRUE,
          legend="right", legend.grob = get_legend(acropora_class_plot))
