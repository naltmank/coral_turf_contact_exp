...
# mixed gamma reg
acropora_gamma <- glmer(Shannon ~ Algal.trt  + (1| Coral.colony.ID) , data = acropora_adiv,
                       family = Gamma(link = "log") )
summary(acropora_gamma)
Anova(acropora_gamma) # X2 = 38.2 P = 1.0e-06
...
# poisson richness
acropora_poisson = glmer(Observed ~ Algal.trt  + (1| Coral.colony.ID) , data = acropora_adiv,
...
# nb richness
acropora_nb = glmer.nb(Observed ~ Algal.trt  + (1| Coral.colony.ID) , data = acropora_adiv ) 
summary(acropora_nb)
Anova(acropora_nb) # X2= 41.5, P = 2.3e-07
plot(residuals(acropora_nb) ~ predict(acropora_nb)) # way better
hist(residuals(acropora_nb)) # better
...
### Plot - figure 5 alpha diversity  acropora
...
# Subsample reads
# (porites_rare <- phyloseq::rarefy_even_depth(porites, rngseed = 123, replace = FALSE))
# head(phyloseq::sample_sums(porites_rare))
...
# mixed gamma reg
porites_gamma = glmer(Shannon ~ Algal.trt  + (1| Coral.colony.ID) , data = porites_adiv,
                       family = Gamma(link = "log") )
summary(porites_gamma)
Anova(porites_gamma) # X2 = 15.8  P = 0.02
plot(residuals(porites_gamma) ~ predict(porites_gamma))
plot(allEffects(porites_gamma))
...
# porites poisson
porites_poisson = glmer(Observed ~ Algal.trt  + (1| Coral.colony.ID) , data = porites_adiv,
                       family = poisson(link = "log") )
summary(porites_poisson)
Anova(porites_poisson) # X2 = 796.3  P < 2.2e-16
...
# mylegend = g_legend(acropora_observed_ggplot)
...
#                c(5, 5, 5, 5))
# 
# grid.arrange(acropora_observed_ggplot + theme(legend.position = "none"), 
#              acropora_adiv_ggplot+ theme(legend.position = "none"), 
#              porites_observed_ggplot+ theme(legend.position = "none"), 
#              porites_adiv_ggplot+ theme(legend.position = "none"),
#              mylegend, layout_matrix = layout
#              )
# 
...
# why is there so much empty space now but I can't make it look right without the height = 45??
# Remember to crop in preview if regenerating figs... so dumb. 
...
# acropora_bray_matrix_qiime = read_qza("/Users/noamaltman-kurosaki/iCloud Drive (Archive) - 1/Documents/Grad school (general)/PhD General/Coral-algal contact experiment/Corrected_files_for_real/acropora_core_metrics_results_final/bray_curtis_distance_matrix.qza")$data
# weighted = list(acropora_bray_matrix, acropora_bray_matrix_qiime) 
# compare_dists(weighted) # confirmed that qiime just uses untransoformed relative abundance for bray curtis

# replace "-" in rownames from qiime with "." for parity
# names(acropora_bray_matrix_qiime) = gsub("-", ".", names(acropora_bray_matrix_qiime)) 

# names(acropora_bray_matrix_qiime) = names(acropora_bray_matrix)
...
# write.csv(acropora.bray.p.table, "/Users/noamaltman-kurosaki/iCloud Drive (Archive) - 1/Documents/Grad school (general)/PhD General/Coral-algal contact experiment/acropora pairwise anosim bray sqrt transformed.csv")
...
# write.csv(acropora.bray.p.table, "/Users/noamaltman-kurosaki/iCloud Drive (Archive) - 1/Documents/Grad school (general)/PhD General/Coral-algal contact experiment/acropora pairwise adonis bray sqrt transformed.csv")
...
# write.csv(porites.bray.p.table, "/Users/noamaltman-kurosaki/iCloud Drive (Archive) - 1/Documents/Grad school (general)/PhD General/Coral-algal contact experiment/porites pairwise anosim bray sqrt transformed.csv")
...
# write.csv(porites.bray.p.table, "/Users/noamaltman-kurosaki/iCloud Drive (Archive) - 1/Documents/Grad school (general)/PhD General/Coral-algal contact experiment/porites pairwise adonis bray sqrt transformed.csv")

porites_bray = ordinate(porites_sqrt, distance = "bray", method = "PCoA")

# porites_sqrt1 <- porites_bray$values$Eigenvalues[1] / sum(porites_bray$values$Eigenvalues)
# porites_sqrt2 <- porites_bray$values$Eigenvalues[2]  / sum(porites_bray$values$Eigenvalues)
...
acropora_deseq_summary = read.csv("/Users/noamaltman-kurosaki/iCloud Drive (Archive) - 1/Documents/Grad school (general)/PhD General/Coral-algal contact experiment/Acropora deseq summary matrix v1.csv", row.names = 1)
...
porites_deseq_summary = read.csv("/Users/noamaltman-kurosaki/iCloud Drive (Archive) - 1/Documents/Grad school (general)/PhD General/Coral-algal contact experiment/porites deseq summary matrix v1.csv", row.names = 1)
...
acropora_deseq = read.csv("/Users/noamaltman-kurosaki/iCloud Drive (Archive) - 1/Documents/Grad school (general)/PhD General/Coral-algal contact experiment/Acropora just genus deseq.csv", stringsAsFactors = T)
...
# total of unique genera that involve sulfur cycling = 4 + hypnocyclus = 5


porites_deseq = read.csv("/Users/noamaltman-kurosaki/iCloud Drive (Archive) - 1/Documents/Grad school (general)/PhD General/Coral-algal contact experiment/Porites just genus deseq.csv", stringsAsFactors = T)
...
# Total genera that involve sulfur cycling = 12 + hypnocyclus = 13
...
```{r acropora unifrac rbiom}

acropora_unifrac_matrix = rbiom::unifrac(acropora@otu_table, weighted=TRUE, tree=acropora@phy_tree)
acropora_unifrac_matrix_qiime = read_qza("/Users/noamaltman-kurosaki/iCloud Drive (Archive) - 1/Documents/Grad school (general)/PhD General/Coral-algal contact experiment/Corrected_files_for_real/acropora_core_metrics_results_final/weighted_unifrac_distance_matrix.qza")$data
weighted = list(acropora_unifrac_matrix, acropora_unifrac_matrix_qiime) 
compare_dists(weighted) # there are the same matrix

# replace "-" in rownames from qiime with "." for parity
names(acropora_unifrac_matrix_qiime) = gsub("-", ".", names(acropora_unifrac_matrix_qiime)) 

names(acropora_unifrac_matrix_qiime) = names(acropora_unifrac_matrix)

acropora_unifrac = pcoa(acropora_unifrac_matrix)
acropora_unifrac_qiime = pcoa(acropora_unifrac_matrix_qiime)
# acropora_unifrac_qiime$values
# Scree plot
phyloseq::plot_scree(acropora_unifrac) + 
  geom_bar(stat="identity", fill = "blue") +
  labs(x = "\nAxis", y = "Proportion of Variance\n")


plot_ordination(acropora, acropora_unifrac, type = "samples", color = "Algal.trt")  + 
  theme_bw() +
  labs(title = "Acropora unifrac\n") +
  geom_point(size = 2) +
#  coord_fixed(acropora_unifrac2 / acropora_unifrac1) +
  scale_color_manual(values = c(Empty = "grey",
               `Turf mimic` = "black",
               `Macroalgae mimic` = "green",
               Amansia = "red",
  ...
  theme(panel.background = element_blank(), 
        ...
        panel.grid.minor = element_blank(),  #remove minor-grid labels
        plot.background = element_blank()) # +
#  stat_ellipse(aes(group = Algal.trt), linetype = 2)
# 27 and 33% on qiime vs 40.6 and 36.2 here

plot_ordination(acropora, acropora_unifrac_qiime, type = "samples", color = "Algal.trt")  + 
  theme_bw() +
  labs(title = "Acropora unifrac\n") +
  geom_point(size = 2) +
#  coord_fixed(acropora_unifrac2 / acropora_unifrac1) +
...
anosim(acropora_unifrac_matrix, phyloseq::sample_data(acropora)$Algal.trt) # R = 0.065, P = 0.004
...
acropora.unifrac.p <- c() # Create vector for outputting P values

for(i in 1:ncol(cbn)){ # for each possible comparison
  ps.subs <- subset_samples(acropora, Algal.trt %in% cbn[,i]) # subset the data in the trt for that comparison
  metadata_sub <- data.frame(sample_data(ps.subs)) # specify the metadata for that subset
  
  permanova_pairwise <- anosim(rbiom::unifrac(ps.subs@otu_table, weighted=TRUE, tree=ps.subs@phy_tree),
                               metadata_sub$Algal.trt) # perform anosim for the data based on those two treatments
  acropora.unifrac.p <- c(acropora.unifrac.p, permanova_pairwise$signif[1]) # append the p value for that comparison to the created p value dataframe
}

acropora.unifrac.p.adj <- p.adjust(acropora.unifrac.p, method = "bonferroni") # adjust the p value for multiple comparisons
acropora.unifrac.p.table <- as.data.frame(cbind.data.frame(t(cbn), p=acropora.unifrac.p, p.adj=acropora.unifrac.p.adj)) # create data table containing p value comparisons 
# NO SIG RESULTS
# write.csv(acropora.unifrac.p.table, "/Users/noamaltman-kurosaki/iCloud Drive (Archive) - 1/Documents/Grad school (general)/PhD General/Coral-algal contact experiment/acropora pairwise anosim unifrac rbiom.csv")


#Dispersion test
acropora_dispr <- vegan::betadisper(acropora_unifrac_matrix, phyloseq::sample_data(acropora)$Algal.trt)
acropora_dispr # 

# plot of dispersion test
plot(acropora_dispr, main = "Ordination Centroids and Dispersion Labeled: Aitchison Distance", sub = "")
boxplot(acropora_dispr, main = "", xlab = "") # turf trts generally furthest from centroid
vegan::permutest(acropora_dispr) # F = 0.6, P = 0.75
...
```{r porites unifrac rbiom}
# issues with unifrac if using phyloseq
# See: https://github.com/joey711/phyloseq/issues/956
# Using rbiom package for calculating unifrac distances instead

porites_unifrac_matrix = rbiom::unifrac(porites@otu_table, weighted=TRUE, tree=porites@phy_tree)
porites_unifrac = pcoa(porites_unifrac_matrix)

# Scree plot
phyloseq::plot_scree(porites_unifrac) + 
  geom_bar(stat="identity", fill = "blue") +
  labs(x = "\nAxis", y = "Proportion of Variance\n")


plot_ordination(porites, porites_unifrac, type = "samples", color = "Algal.trt")  + 
  theme_bw() +
  labs(title = "porites unifrac\n") +
  geom_point(size = 2) +
#  coord_fixed(porites_unifrac2 / porites_unifrac1) +
  scale_color_manual(values = c(Empty = "grey",
               `Turf mimic` = "black",
               `Macroalgae mimic` = "green",
               Amansia = "red",
  ...
  theme(panel.background = element_blank(), 
        ...
        panel.grid.minor = element_blank(),  #remove minor-grid labels
        plot.background = element_blank()) 
# 28.6 and 12.9 on qiime vs 29.3 and 13.3 on qiime

anosim(porites_unifrac_matrix, phyloseq::sample_data(porites)$Algal.trt) # R = 0.13, P = 0.001
...
porites.unifrac.p <- c() # Create vector for outputting P values
...
for(i in 1:ncol(cbn)){ # for each possible comparison
  ...
  permanova_pairwise <- anosim(rbiom::unifrac(ps.subs@otu_table, weighted=TRUE, tree=ps.subs@phy_tree),
                               metadata_sub$Algal.trt) # perform anosim for the data based on those two treatments
  porites.unifrac.p <- c(porites.unifrac.p, permanova_pairwise$signif[1]) # append the p value for that comparison to the created p value dataframe
}

porites.unifrac.p.adj <- p.adjust(porites.unifrac.p, method = "bonferroni") # adjust the p value for multiple comparisons
porites.unifrac.p.table <- as.data.frame(cbind.data.frame(t(cbn), p=porites.unifrac.p, p.adj=porites.unifrac.p.adj)) # create data table containing p value comparisons
# SN, SP, and Sarg all different from empty. SP also different from TM
# write.csv(porites.unifrac.p.table, "/Users/noamaltman-kurosaki/iCloud Drive (Archive) - 1/Documents/Grad school (general)/PhD General/Coral-algal contact experiment/porites pairwise anosim unifrac rbiom.csv")
...
plot(porites_dispr, main = "Ordination Centroids and Dispersion Labeled: Aitchison Distance", sub = "")
boxplot(porites_dispr, main = "", xlab = "") # turf trts generally furthest from centroid
vegan::permutest(porites_dispr) # F = .8, P = 0.5
